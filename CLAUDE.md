...
| **PreToolUse** *(optional, off by default)* | `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow\|deny\|ask",...}}` | **Not wired in the default install** — opt-in only (see *Hook wiring* below). When enabled, default config is **terminal-mirror mode**: `DangerPattern = '.'` (matches every command) and the permission-mode gate is commented out, so the widget shows for any command **not** covered by a `permissions.allow` rule, in every permission mode. Because the hook input carries `permission_mode` but **not** the resolved permission decision (and can't replicate the safe-command/auto classifier or session-scoped approvals), this over-fires vs. the terminal in `plan`/`auto`/`bypassPermissions`. To narrow it: re-enable the mode gate (commented one-liners in the PreToolUse block — `default`-only, or exclude only never-prompting modes) and/or set a real `DangerPattern` regex in `config.psd1`. Allow rules must use the `Bash(...)` tool form since the hook `matcher` is `Bash`. | Claude Code now routes **subagent** tool calls through the same PreToolUse hook (input carries `agent_id`/`agent_type`), so `Test-CommandAllowed` mirrors the allowlist — reading `permissions.allow` from `~/.claude` + `$CLAUDE_PROJECT_DIR\.claude` (`settings.json` & `settings.local.json`) — to keep the widget from popping for commands the terminal would silently allow. Since the hook runs *before* allow rules, a widget "deny" still blocks an allowlisted command (deny/ask *rules* would override, but plain allow rules don't). |
